...
const MIN_VOL = 500000; // $500K mínimo en ambos DEX
...
// Fetch histórico de funding rate de HL para un token
...
function SpreadChart({ symbol, shortDex, longDex, hlMap, asterMap, backMap }) {
  ...
  useEffect(() => {
    ...
    fetchHLHistory(symbol, days).then(data => {
      // data = [{time, fundingRate}, ...]
      // El spread = HL funding - otro DEX funding (approx constante por ahora)
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ display: "flex", gap: 20, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <div style={{ width: 16, borderTop: `2px dashed ${T.blue}` }} />
            <span style={{ color: T.blue, fontSize: 11, fontWeight: 600 }}>Media</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <div style={{ width: 10, height: 10, background: T.green, borderRadius: 2 }} />
            <span style={{ color: T.muted, fontSize: 11 }}>Spread APR</span>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ color: T.subtle, fontSize: 13, textAlign: "center", padding: 20 }}>Sin datos históricos disponibles</div>
      ) : (
        <div style={{ overflowX: "auto" }}>
          <svg width={W} height={H} style={{ display: "block" }}>
            {[0, 25, 50, 75, 100].map(p => {
              const v = yMin + (yMax - yMin) * p / 100;
              const y = toY(v);
              return (
                <g key={p}>
                  <line x1={PAD.left} y1={y} x2={W-PAD.right} y2={y} stroke={T.border} strokeWidth={1}/>
                  <text x={PAD.left-6} y={y+4} fill={T.subtle} fontSize={9} textAnchor="end">{v.toFixed(0)}%</text>
                </g>
              );
            })}
            {history.map((h, i) => {
            ...
            {[0, Math.floor(history.length/2), history.length-1].map(i => i < history.length && (
              <text key={i} x={PAD.left+(i/history.length)*IW+barW/2} y={H-PAD.bottom+14}
                fill={T.subtle} fontSize={8} textAnchor="middle">
  ...
  const runScan = useCallback(async () => {
    ...
    try {
      ...
      for (const sym of Object.keys(hlMap)) {
        ...
        // FILTRO DURO: $500K mínimo en ambos DEX
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div>
          ...
          {lastScan && (
            <div style={{ color: T.subtle, fontSize: 12, marginTop: 4 }}>
              {lastScan.toLocaleTimeString()} · {results.length} pares con vol suficiente
          ...
          {results.map((r, i) => {
            ...
            return (
              <div key={r.symbol}>
                ...
                {/* Detalle expandible con gráfico */}
                {isSelect && (
                  <div style={{ padding: "20px", background: "#f8fafc", borderBottom: `1px solid ${T.border}` }}>
                    ...
                    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: 16 }}>
                      ...
                      <SpreadChart
                        ...
                        longDex={r.longDex}
                        hlMap={maps.hlMap || {}}
                        asterMap={maps.asterMap || {}}
                        backMap={maps.backMap || {}}
                      />
                    </div>
                  </div>
                )}
      ...
      {results.length > 0 && (
        <div style={{ color: T.subtle, fontSize: 12, textAlign: "center" }}>
          {results.length} tokens con vol {'>'} $500K en ambos DEX · Clic en fila para historial
        </div>
      )}
    </div>
  );
