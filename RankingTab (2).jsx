...
const MIN_VOL_HL = 1000000; // $1M en HL (el más líquido)
...
async function fetchBackpack() {
  try {
    // Backpack ticker endpoint correcto
    const tickerRes = await proxyGet(`${BACK_API}/ticker`);
    const tickers = Array.isArray(tickerRes) ? tickerRes : [];
    const map = {};
    tickers.forEach(t => {
      if (!t.symbol?.includes("PERP")) return;
      const sym   = t.symbol.replace("_USDC_PERP", "").replace("_USDT_PERP", "").replace("_USD_PERP", "");
      const price = parseFloat(t.lastPrice || t.markPrice || 0);
      if (!price) return;
      map[sym] = {
        dex: "Backpack",
        fundingApr: parseFloat(t.fundingRate || 0) * 24 * 365 * 100, // hourly rate
        price,
        vol24h: parseFloat(t.quoteVolume || t.volume || 0),
        oi:     0,
        spreadPct: price > 0 ? (parseFloat(t.askPrice || price*1.001) - parseFloat(t.bidPrice || price*0.999)) / price : 0,
      };
    });
    return map;
  } catch { return {}; }
...
async function fetchHLHistory(symbol, days) {
  try {
    ...
    const data = await proxyPost(HL_API, {
      type: "fundingHistory",
      coin: symbol,
      startTime: start,
    });
...
export default function RankingTab({ config }) {
  ...
  const runScan = useCallback(async () => {
    ...
    try {
      ...
      setDebugInfo(`Back: ${backSample || "vacío"}`);
      ...
      for (const sym of Object.keys(hlMap)) {
        const hlData = hlMap[sym];

        // Filtro: vol HL mínimo $1M
        if (hlData.vol24h < MIN_VOL_HL) continue;
        ...
        const vol       = hlData.vol24h;
        const oi        = hlData.oi || 0;
      ...
      setDebugInfo(prev => `${prev} | ${results.length} pares con HL vol>$1M`);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <div style={{ color: T.subtle, fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>
            RANKING · HL × ASTER × BACKPACK · HL VOL &gt; $1M
          </div>
          {lastScan && (
      ...
      {loading && (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 40, textAlign: "center" }}>
          ...
          <div style={{ color: T.subtle, fontSize: 13 }}>HL · Aster · Backpack · 20-30 segundos</div>
        </div>
      )}
      ...
      {!loading && results.length === 0 && !error && lastScan && (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 40, textAlign: "center" }}>
          ...
          <div style={{ color: T.muted, fontSize: 15 }}>Sin oportunidades ahora mismo</div>
      ...
      {!loading && results.length === 0 && !error && !lastScan && (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 60, textAlign: "center" }}>
          ...
          <div style={{ color: T.subtle, fontSize: 13, marginTop: 6 }}>HL × Aster × Backpack · HL vol &gt; $1M</div>
        </div>
      )}
      ...
      {results.length > 0 && (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "36px 90px 64px 110px 110px 90px 90px 60px", padding: "12px 16px", borderBottom: `1px solid ${T.border}`, background: "#f8fafc" }}>
            {["#", "TOKEN", "SCORE", "SHORT", "LONG", "SPREAD APR", "VOL HL", "DEX"].map(h => (
            ...
            return (
              <div key={r.symbol}>
                <div onClick={() => setSelected(isSelect ? null : r)} style={{
                  ...
                  <div style={{ color: T.green, fontSize: 12, fontWeight: 600 }}>${(r.vol/1000000).toFixed(1)}M</div>
                ...
                {isSelect && (
                  <div style={{ padding: "20px", background: "#f8fafc", borderBottom: `1px solid ${T.border}` }}>
                    ...
                    <div style={{ display: "flex", gap: 16, flexWrap: "wrap", fontSize: 12, color: T.muted, marginBottom: 16 }}>
                      ...
                      <span>OI HL: <b>${(r.oi/1000000).toFixed(1)}M</b></span>
                    </div>
      ...
      {results.length > 0 && (
        <div style={{ color: T.subtle, fontSize: 12, textAlign: "center" }}>
          {results.length} tokens · HL vol &gt; $1M · Clic en fila para historial
        </div>
      )}
    </div>
  );
